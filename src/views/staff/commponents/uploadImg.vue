<template>
  <div>
    ...
    <el-dialog :visible.sync="showDialog" title="图片预览">
      <img :src="imgUrl" alt="" style="width:60%">
    </el-dialog>
...
const cos = new COS({
  ...
  SecretId: 'AKIDsZz93g16uRhgPr2vYuznLcGdjY6NN6Yj', // 身份识别 ID
  SecretKey: 'vhCQN3SaS3NQCizyg7HAmAH8i12nHTO2' // 身份密钥
...
export default {
  ...
  watch:{
    ...
    img:function(){
      ...
      this.fileList.push({url:this.img})
      }
    }
  },
  methods: {
          ...
          if (!err && data.statusCode === 200) {
            ...
            this.fileList = this.fileList.map(item => {
              ...
              if (item.uid === this.currentFileUid) {
                  //保存上传图片的地址，传递给父组件
                  const imgAdress = 'http://'+data.Location
              console.log('http://'+data.Location)
               this.$emit('upload',imgAdress)
                // 将成功的地址赋值给原来的url属性
